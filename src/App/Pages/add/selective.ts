...
import {
  SelectTreeBranch,
  SelectTree,
} from "../../../Terminal/User Interactions/select-tree";
...
export class SelectiveAddPage implements AppPage {
  ...
  findBranchByName(
    name: string,
    inBranches: SelectTreeBranch[]
  ): SelectTreeBranch | null {
  ...
  async run(): Promise<AppPage | null> {
    ...
    const statusResponse = await this.app.gitCommand
    ...
    const pathsToAdd = response.selectedItems.map((item) =>
      this.getPathForBranch(item)
    );

    const pathsToReset = response.unselectedItems.map((item) =>
      this.getPathForBranch(item)
    );

    for (let path of pathsToAdd) {
      await this.app.gitCommand
        .clear()
